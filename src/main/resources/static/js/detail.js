    ...
    $.ajax({
        ...
        success: function (data, status) {
            if (status == "success") {
                ...
                // 댓글 목록을 불러오고 난 뒤 삭제에 대한 이벤트 리스너를 등록해야 한다
                addDelete();
            }
        },
    })
...
// 댓글 삭제 버튼이 눌렸을 때 . 해당 댓글 삭제하는 동작을 이벤트 핸들러로 등록
function addDelete() {
    //현재 글의 id
    const id = $("input[name='id']").val().trim();
