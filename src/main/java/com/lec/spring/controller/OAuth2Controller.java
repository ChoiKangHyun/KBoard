...
public class OAuth2Controller {
    ...
    private void loginKakaoUser(User kakaoUser) {
        ...
        U.getSession().setAttribute(HttpSessionSecurityContextRepository.SPRING_SECURITY_CONTEXT_KEY,securityContext);
    ...
    private User registerKakaoUser(KakaoProfile profile) {
        //새로 가입시킬 username을 생성(unique 해야 한다)
        ...
        if (user == null) { // 미가입자 인 경우 회원 가입 진행
            User newUser = User.builder()
                    ...
                    .build();
            int count = userService.register(newUser);
            if (count > 0) {
    ...
    private KakaoProfile kakaoUserInfo(String accessToken) {
        ...
        // body 는 불필요. header만 담은 HttpEntity 생성
        ...
        System.out.println("카카오 사용자 Profile 요청 body:" + response.getBody());

        // 사용자 정보 JSON-> Java 로 받아 내기
    ...
    public KakaoOAuthToken kakaoAccessToken(String code) {
        // POST 방식 key - value 형식으로 데이터 요청 (카카오 서버 쪽으로)
        RestTemplate restTemplate = new RestTemplate();
